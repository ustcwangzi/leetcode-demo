 ...
 * These segments can be represented by the 2D array painting where painting[j] = [leftj, rightj, mixj] describes a half-closed
 ...
 *
 * Example 1:
 * @link ../../../../resource/DescribeThePainting1.jpg
 ...
 *
 * Example 2:
 * @link ../../../../resource/DescribeThePainting2.jpg
 ...
 *
 * Example 3:
 * @link ../../../../resource/DescribeThePainting3.jpg
...
public class DescribeThePainting {
    ...
    /**
     * 扫描线 + TreeMap
     * 把 start、end 当成独立的事件全部打散，按照时间排序，对 start 加上 delta，对 end 减去 delta
     * 然后遍历 TreeMap，将结果依次收集到结果集中
     */
    public static List<List<Long>> splitPainting(int[][] segments) {
        ...
        for (int key : map.keySet()) {
            if (sum > 0) {
                result.add(Arrays.asList(start, (long) key, sum));
            }
            sum += map.get(key);
            start = key;
        }
