...
public class CountTheHiddenSequences {
    ...
    /**
     * 假设第一个元素是 0，根据 differences[] 可以依次计算出其他元素，其中最大值、最小值分别是 max、min，那么
     * 若 upper - lower < max - min，无法构满足条件的数组
     * 若 upper - lower == max - min，能构成 1 个满足条件的数组
     * 若 upper - lower == max - min + 1，能构成 2 个满足条件的数组
     * 若 upper - lower == max - min + k，能构成 k+1 个满足条件的数组
     *
     * 假设第一个元素是 X，differences 为 [3, -4, 5, 1, -2]，那么计算得到的数组为 [X, X+3, X-1, X+4, X+5, X+3]
     * 也就是要找到所有可能的 X，使得 X+5 <= upper && X-1 >= lower
     */
