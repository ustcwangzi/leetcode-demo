...
public class CreateSortedArrayThroughInstructions {
    ...
    public static int createSortedArray(int[] instructions) {
        ...
        for (int num : instructions) {
            int left = bit.getSumRange(0, num - 1);
            int right = bit.getSumRange(num + 1, max);
            cost = (cost + Math.min(left, right)) % mod;
            // Increase count of number num by one
            bit.addValue(num, 1);
        }
        return cost;
    }
    ...
    private static class BIT {
        ...
        /**
         * 求从数组 [0...idx] 所有数字的和，1-based indexing
         */
        ...
        /**
         * 求数组 [left, right] 所有数字的和，1-based indexing
         */
