...
public class AverageWaitingTime {
    ...
    /**
     * 记录每餐完成时间 finishTime，遍历 customers
     * 如果顾客到店时间 >= finishTime，说明可以直接做，finishTime 就是顾客到店时间+做餐时间，而等待时间就是做餐时间
     * 否则，说明上一个还没做完，不能直接做，finishTime 就是上一次的 finishTime + 本次做餐时间，而当前 finishTime - 到店时间
     */
    public static double averageWaitingTime(int[][] customers) {
        double finishTime = 0, waitTime = 0;
        for (int[] customer : customers) {
            if (customer[0] >= finishTime) {
                // 直接做
