...
public class SplittingStringIntoDescendingConsecutiveValues {
    ...
    /**
     * 回溯，对每一个当前值 cur，寻找是否存在 cur+1
     */
    public static boolean splitString(String s) {
        return helper(s, null);
    }

    private static boolean helper(String s, BigInteger pre) {
        // 遍历每个分割点
        for (int i = 1; i < s.length(); i++) {
            BigInteger cur = new BigInteger(s.substring(0, i));
            // 不满足条件，跳过
            if (pre != null && !cur.add(BigInteger.ONE).equals(pre)) {
                continue;
            }
            // 满足条件，继续向下寻找
            if (helper(s.substring(i), cur)) {
                return true;
            }
        }

        return (new BigInteger(s)).add(BigInteger.ONE).equals(pre);
    }
}
