...
public class MinimumCostForTickets {
    ...
    /**
     * dp[i] 表示旅行到 i 天需要的最少旅行价格
     * dp[i] = dp[i−1]，如果第 i 天没有行程
     * dp[i] = min{costs[0]+dp[i−1], costs[1]+dp[i−7], costs[2]+dp[i−30]}，如果第 i 天有行程
     */
    public static int mincostTickets(int[] days, int[] costs) {
        int n = days.length, maxDay = days[n - 1];
        ...
        for (int i = 1; i <= maxDay; i++) {
            ...
            int seven = costs[1] + (i >= 7 ? dp[i - 7] : 0);
            int thirty = costs[2] + (i >= 30 ? dp[i - 30] : 0);
