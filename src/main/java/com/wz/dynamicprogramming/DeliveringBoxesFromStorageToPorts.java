 ...
 * 3. 1 <= portsi <= portsCount
 * 4. 1 <= weightsi <= maxWeight
 */
public class DeliveringBoxesFromStorageToPorts {
    public static void main(String[] args) {
        System.out.println(boxDelivering(new int[][]{{1, 1}, {2, 1}, {1, 1}}, 2, 3, 3));
        System.out.println(boxDelivering(new int[][]{{1, 2}, {3, 3}, {3, 1}, {3, 1}, {2, 4}}, 3, 3, 6));
    }

    /**
     * 一次装运处理箱子比任意两次装运需要的跑路次数少，因此需要
     * 1.尽量多装少趟
     * 2.不影响跑路次数的情况下，最后一次考虑尽量少装
     * 使用滑动窗口处理一趟最多能装运的最多箱子数：窗口右界每次 +1，然后处理窗口左界
     * - 重量超过最大载重
     * - 箱子数超过最大载箱
    ...
    public static int boxDelivering(int[][] boxes, int portsCount, int maxBoxes, int maxWeight) {
        int n = boxes.length, weight = boxes[0][1], box = 1, trip = 2;
        int[] dp = new int[n + 1];
        dp[1] = 2;
        for (int left = 0, right = 1; right < n; right++) {
            ...
            while (weight > maxWeight || box > maxBoxes || dp[left + 1] == dp[left]) {
                box--;
                weight -= boxes[left++][1];
